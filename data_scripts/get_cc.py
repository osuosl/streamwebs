...
for dir in directions:
  if os.path.isdir("../streamwebs_frontend/sw_data/"):
    datafile = '../sw_data/' + dir + '_cc.csv'
  else:
    datafile = '../csvs/' + dir + '_cc.csv'
      ...
      if row['Delta'] == '':
        if row['Nid'] in CCs:
          ...
          if row['Collected'][16] == '0' or row['Collected'][16] == '1' or row['Collected'][16] == '2':
            cc.date_time = datetime.strftime(row['Collected'], "%a, %Y-%m-%d %H:%M")
          else:
            cc.date_time = datetime.strftime(row['Collected'][0:15], "%a, %Y-%m-%d")
        CCs[row['Nid']] = cc
        count = 0
